...
export default function HotspotProjectionPanel({
  ...
  const hotspotsData = useMemo(() => {
    ...
    const now = new Date();
    const horizonCutoff = new Date(now.getTime() - (projectionHorizon === "24h" ? 1 : projectionHorizon === "7d" ? 7 : 30) * 24 * 60 * 60 * 1000);
    ...
    const resolvedHotspots: SpatialHotspot[] = clusters.map((cluster, index) => {
      let score = 0;
      let crit = 0;
      let high = 0;
      let med = 0;
      ...
      // Safeguard names
      if (locationLabel.toLowerCase().includes("saskatoon") || locationLabel.toLowerCase() === "canada" || locationLabel.toLowerCase() === "sk") {
        locationLabel = "Strategic Sector";
      }
      if (nbName.toLowerCase() === "saskatoon" || nbName.toLowerCase() === "canada" || nbName.toLowerCase() === "sk") {
    ...
    // 4. Sort by density score descending and return top 5
